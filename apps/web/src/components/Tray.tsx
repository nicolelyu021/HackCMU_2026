...
export function Tray({
  ...
  return (
    <Panel className={cx('w-[340px] overflow-hidden', !open && 'w-auto')}>
      ...
      {open && (
        <ul className="max-h-[38vh] space-y-2 overflow-y-auto px-3 pb-3">
          {items.map((m) => (
            <li key={m.id} className="rounded-xl border border-slate-200 bg-white p-2">
              <div className="flex items-center gap-2.5">
                <img
                  ...
                  alt=""
                  className="h-14 w-14 rounded-lg object-cover bg-slate-200"
                />
                <div className="min-w-0 flex-1 text-xs text-slate-600">
                  <div className="truncate font-medium text-slate-800">
                    {m.taken_at
                      ? `${m.taken_at.slice(0, 10)} · ${clock(m.taken_at)}`
                      : 'no timestamp'}
                  </div>
                  <div>
                    {m.lat !== null
              ...
              {picking === m.id && (
                <div className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-lg bg-slate-50 p-2">
                  {pinsByDay.map((d) => (
                    <div key={d}>
                      ...
                      {bundle.pins
                        ...
                        .map((p) => (
                          <button
                            key={p.id}
                            className="flex w-full items-center gap-2 rounded-md px-1.5 py-1 text-left text-xs hover:bg-white"
