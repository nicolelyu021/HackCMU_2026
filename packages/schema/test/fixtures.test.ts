...
describe('demo fixtures satisfy the contracts', () => {
  ...
  it('has 21 photos: 19 assigned, 2 in the tray, all expected pins exist', () => {
    expect(f.media).toHaveLength(21);
    const pinIds = new Set(f.pins.map((p) => p.id));
    expect(f.media.filter((m) => m.pin_id).length).toBe(19);
    ...
    expect(f.media.filter((m) => m.pin_id === 'pin_pgh_d2_cmu')).toHaveLength(3);
  });
